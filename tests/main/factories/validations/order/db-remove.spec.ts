...
function makeSut() {
  ...
  const sut = makeRemoveOrderValidation(validationService)

  return { validationService, sut }
}

describe(makeRemoveOrderValidation.name, () => {
  describe.each([
    // id
    {
    ...
    ({ properties, validations }) => {
      it(JSON.stringify(validations), async () => {
        ...
        const requestModel = { ...properties } as RemoveOrderUseCase.RequestModel
